 ...
 * påverkar i sin tur dessa. Detta betyder att den troligtvis kommer att behöva
 * uppdateras flera gånger under kursens gång.
 *
 * @implNote För den som har erfarenhet av JUnit och undrar varför vi inte
 * använder de vanliga assert-metoderna (assertEquals etc.) i testerna så beror
 * det på att de blir väldigt klumpiga i flera situationer som är ovanliga i
 * vanlig testning, men förekommer i detta specifika sammanhang, till exempel
 * när det finns flera möjliga alternativ som är korrekta.
 * <p>
 * Ett matcherbibliotek som hamcrest skulle fungerat, men också introducerat ett
 * beroende som tvingat alla studenter att ladda ner och installera biblioteket.
 * <p>
 * Ett eget matcherbibliotek ger också möjlighet att använda JavaDoc för att ge
 * dokumentation för de olika delstegen i utvärderingen.
 * <p>
 * Anledningen till att vi använder arv istället för statiska importer är att de
 * sistnämnda kräver paket, som inte tas upp på PROG1.
 *
 */
public abstract class DR01Assertions {

    /**
     * Hur lång tid ett test får ta på sig innan det automatiskt anses
     * misslyckat (50ms).
     */
     ...
     */
    /**
     * Klassnamn inleds med stor bokstav, och har stor bokstav på varje nytt
     * ord. De kan inledas med ett paketnamn.
     */
     ...
     * <code>ENUM$VALUES</code>. Den ska <u>aldrig</u> användas i dina egna namn
     * på konstanter.
     */
    private static final Pattern CONSTANT_NAMING_PATTERN = Pattern.compile("[A-Z]+([_\\$][A-Z0-9]+)*");

    /**
     * Variabler och metoder inleds med liten bokstav och har stor bokstav på
     * varje nytt ord.
     *
     * @implNote $ läggs in i vissa namn som Java-kompilatorn skapar, tex namn
     * på lamda-uttryck.
     */
    ...
    /**
     * Fält i InputReader som används för att kontrollera att bara en instans
     * skapas. Ligger här eftersom den behövs flera test.
     */
    ...
    /**
     * Det ursprungliga värdet för fältet ovan, används för att resetta
     * kontrollen när testen behöver skapa flera instanser. Ligger här eftersom
     * den behövs flera test.
     */
    ...
    /**
     * Kontrollera att testet inte tar för lång tid på sig att köra. Om detta
     * test misslyckas beror det antagligen på en oändlig loop eller rekursion i
     * ditt program, eller på att det står och väntar på inmatning från
     * användaren som aldrig kommer.
     * <p>
     * Tiden som accepteras är generös, men det går aldrig att <u>helt</u>
     * garantera att ett misslyckande inte beror på att datorn är upptagen med
     * något annat som tar alla resurser. Risken för detta är dock <u>mycket</u>
     * liten om du inte märker själv att hela datorn verkar (mycket) långsam.
     * <p>
     * Om du trots ovanstående varning är övertygad om att det är timeout-tiden
     * som är för kort, och inte din kod som är fel, så kan du testa att öka
     * tiden som accepteras genom att ändra konstanten <code>TEST_TIMEOUT</code>
     * i <code>DR00Assertions</code>.
     *
     * @see #TEST_TIMEOUT
     */
    ...
    /**
     * Kontrollera att de grundläggande ickefunktionella kraven på skyddsnivåer
     * i uppgiften följs i en given klass. Det som kontrolleras är:
     * <p>
     * <ul>
     ...
     * rätt skyddsnivåer används på metoder. En sådan kontroll skulle kräva att
     * testet förstod metodernas syfte vilket det inte kan göra. Detta kommer
     * därför att kontrolleras i samband med granskningen och den manuella
     * rättningen efter deadline.
     *
     * @param cut klassen vars skyddsnivåer ska kontrolleras. (CUT står för
     * Class Under Test.)
     */
     ...
     * <p>
     * Observera att testet <u>inte</u> har någon möjlighet att kontrollera att
     * namnet är korrekt, tydligt, inteskrivetpådethärsättet, eller något annat
     * som skulle kräva att det faktiskt förstod vad namnen betyder. Detta
     * kommer därför att kontrolleras i samband med granskningen och den
     * manuella rättningen efter deadline.
     * <p>
     * Observera också att testet <u>bara</u> kontrollerar namn på klassnivå,
     * alltså inga namn inuti metoder.
     *
     * @param cut klassen vars skyddsnivåer ska kontrolleras. (CUT står för
     * Class Under Test.)
     */
    ...
    protected static class Value {
        ...
        protected static interface DogRegisterAssertion {
             ...
             * @implNote Denna metod gör samma sak som
             * <code>onErrorReport(String)</code>, men ger tydligare test om
             * felmeddelandet ska formateras.
             */
             ...
             *
             * @implNote Denna metod ska normalt inte användas, om inte
             * anledningen till felet är <b>mycket</b> uppenbart.
             */
        ...
        protected static record DoubleValue(double actual) {
            ...
            protected static record DoubleIsAssertion(double expected, double actual, double tolerance)
                ...
                /**
                 * Acceptera att värdet kan ligga lite mer ifrån det förväntade,
                 * men fortfarande nära.
                 *
                ...
                /**
                 * Acceptera att värdet ligger max så här långt från det
                 * förväntade.
                 */
        ...
        protected static record StringValue(String actual) {
            ...
            protected static record StringIsAssertion(String expected, String actual,
                ...
                /**
                 * Genomför kontrollen utan hänsyn till stora och små bokstäver.
                 * ABC kommer alltså att anses som samma sak som abc.
                 */
        ...
        protected static record ArrayValue<T>(T[] actual) {
            ...
            protected static record ArrayContainsAssertionStep<T>(T[] expected, T[] actual) {
                ...
                /**
                 * Ordningen på elementen är viktig. [a, b] är alltså inte samma
                 * som [b, a].
                 */
                ...
                /**
                 * Gör jämförelsen utan hänsyn till ordningen på elementen i
                 * listan. [a, b] är alltså samma som [b, a].
                 */
            ...
            protected static record ArrayIsInOrderAssertion<T>(T[] expected, T[] actual)
                ...
                public void onErrorReport(String errorMsg) {
                    if (!Arrays.equals(expected, actual)) {
                        assertionFailure().message(errorMsg).expected(Arrays.toString(expected))
                                .actual(Arrays.toString(actual)).buildAndThrow();
                    }
                }
            }
            ...
            protected static record ArrayContainsIgnoringOrderAssertion<T>(T[] expected, T[] actual)
                ...
                public void onErrorReport(String errorMsg) {
                    ...
                    // TODO: Detta villkor hanterar inte dubbletter, och skulle anse [A, A, B] vara
    ...
    /**
     * Denna klass representerar en publik metod som det av någon anledning inte
     * är säkert att den existerar. Det kan vara en obligatorisk metod som man
     * ska designa eller namnge själv, eller en metod som är tillåten men inte
     * obligatorisk.
     */
